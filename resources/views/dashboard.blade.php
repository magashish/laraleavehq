...
<div class="page" x-data="calendar()" x-init="init()">
    ...
    <div class="card" style="margin-bottom:20px;">
        <div style="display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:10px;">
            <div>
                <div class="card-title" style="margin-bottom:2px;">Attendance</div>
                <div style="font-size:12px;color:#888;">{{ now()->format('l, j F Y') }}</div>
            </div>
            <div style="display:flex;align-items:center;gap:12px;flex-wrap:wrap;">
                @if($todayCheckin)
                    <span style="font-size:13px;color:#555;">
                        @if($todayCheckin->signed_out_at)
                            Signed out at <strong>{{ $todayCheckin->signed_out_at->format('H:i') }}</strong>
                        @else
                            Signed in at <strong style="color:#0d6648;">{{ $todayCheckin->checked_in_at->format('H:i') }}</strong>
                        @endif
                    </span>
                @endif
                @if(!$todayCheckin)
                ...
                @else
                    <form method="POST" action="{{ route('checkin.store') }}">
                        @csrf
                        <button type="submit" class="btn btn-outline btn-sm">Sign in again</button>
                    </form>
                @endif
            </div>
        </div>
