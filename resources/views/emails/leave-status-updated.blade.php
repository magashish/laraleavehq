...
<head>
    ...
    <style>
        ...
        .icon-circle { display: inline-flex; align-items: center; justify-content: center; width: 64px; height: 64px; border-radius: 50%; margin-bottom: 16px; }
        .icon-circle svg { width: 34px; height: 34px; }
        .approved .icon-circle, .booked .icon-circle { background: #ddeaf6; }
        .rejected .icon-circle { background: #fde8e8; }
        ...
        .status-banner { margin: 0 24px 20px; padding: 12px 16px; border-radius: 8px; font-size: 13px; font-weight: 500; }
        .approved .status-banner, .booked .status-banner { background: #f5f9ff; color: #000; border-left: 3px solid #83acdb; }
...
<div class="wrapper {{ $statusClass }}">
    ...
    <div class="header">
        <div class="icon-circle">
            @if($isApproved)
                <svg viewBox="0 0 24 24" fill="none" stroke="#83acdb" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round">
                    <circle cx="12" cy="12" r="10"/><polyline points="9 12 11 14 15 10"/>
                </svg>
            @else
                <svg viewBox="0 0 24 24" fill="none" stroke="#d9534f" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round">
                    <circle cx="12" cy="12" r="10"/><line x1="15" y1="9" x2="9" y2="15"/><line x1="9" y1="9" x2="15" y2="15"/>
                </svg>
            @endif
        </div>
        <h1>{{ $isApproved ? 'Leave Approved' : 'Leave Declined' }}</h1>
        <p class="sub">{{ now()->format('j F Y') }}</p>
    </div>

    <div class="greeting">
        <p>Hi {{ $leave->employee->name }},</p>
        @if($isApproved)
            <p>Your leave request has been approved. Here are the details of your booking.</p>
        @else
